...
```mermaid
flowchart LR
  A[Request] --> B[Compression]
  B --> C[PerformanceMiddleware]
  C --> D[ResponseCaching]
  D --> E[StaticFiles]
  E --> F[Routing]
  F --> G[Session]
...
stateDiagram-v2
  ...
  processing --> cancelled: fail/timeout
  completed --> [*]
  cancelled --> [*]
```
